...
def analyze_efficiency(fleet, hub):
    ...
    hub_node = {'lat': hub['lat'], 'lon': hub['lon']}
    
    for v in fleet.values():
        if v['original_path']:
            # Classical: Distance following the order the user added them
            total_classical += route_distance(hub_node, v['original_path'])
            # Quantum: Distance following the optimized solver order
            total_quantum += route_distance(hub_node, v['optimized_path'])
            
    saved = max(total_classical - total_quantum, 0)
    # If the optimizer is perfect, saved will always be >= 0
    fuel = saved * 0.25
    co2 = fuel * 2.31
    time_saved = saved / 40
    
    return total_classical, total_quantum, saved, fuel, co2, time_saved
...
with col_ctrl:
    ...
    if st.button("⚡ Dispatch Fleet", use_container_width=True):
        if not st.session_state.hubs:
            ...
            # Reset all fleet paths
            for v in st.session_state.fleet.values():
                v['original_path'] = []
                v['optimized_path'] = []
                v['path'] = []

            # 1. DISTRIBUTE STOPS (Classical Order)
            # We assign stops to vehicles exactly as they were entered
            vnames = [n for n in st.session_state.fleet.keys() if st.session_state.fleet[n]['status'] != "Broken"]
            if vnames:
                for i, s in enumerate(st.session_state.stops):
                    vname = vnames[i % len(vnames)]
                    st.session_state.fleet[vname]['original_path'].append(s)

            # 2. OPTIMIZE PATHS (Quantum Order)
            for name, v in st.session_state.fleet.items():
                if v['original_path']:
                    # The Quantum solver takes the random classical list and reorders it optimally
                    v['optimized_path'] = solve_routing_qubo([v['start_pos']] + v['original_path'])
                    v['path'] = v['optimized_path'][:]
                    v['status'] = "On Mission"
                    
                    # Optional: Generate road points for map
                    full_road = []
                    curr = v['start_pos']
                    for stop in v['path']:
            ...
            st.success("Quantum Engine has collapsed the multi-stop superposition into an optimal route!")
            st.rerun()
